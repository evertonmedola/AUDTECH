...
@Component({
  selector: 'app-template-global-form',
  standalone: true,
  imports: [
  ...
  template: `
    <app-page-header
      [titulo]="editando() ? 'Editar Template Padrão' : 'Novo Template Padrão'"
      [breadcrumb]="[{ label: 'Templates Padrão', url: '/templates-globais' }, { label: editando() ? 'Editar' : 'Novo' }]"
    />
    ...
    <div class="max-w-3xl">
      <form [formGroup]="form" (ngSubmit)="salvar()">
        ...
        <div cdkDropList (cdkDropListDropped)="reordenar($event)" class="flex flex-col gap-3 mb-6">
          @for (item of itens.controls; track item; let i = $index) {
            <div cdkDrag [formGroup]="getItemGroup(i)"
        ...
        <div class="flex gap-3">
          <button mat-stroked-button type="button" routerLink="/templates-globais">Cancelar</button>
...
export class TemplateGlobalFormComponent implements OnInit {
  ...
  getItemGroup(i: number): FormGroup { return this.itens.at(i) as FormGroup; }
  ...
  ngOnInit(): void {
    ...
    if (this.templateId) {
      ...
      this.templateService.buscar(this.templateId).subscribe(t => {
        ...
        t.itens?.sort((a, b) => a.ordem - b.ordem)
  ...
  adicionarItem(): void {
    this.itens.push(this.fb.group({ grupo: [''], descricao: ['', Validators.required] }));
  }

  private adicionarItemExistente(grupo: string, descricao: string): void {
    this.itens.push(this.fb.group({ grupo: [grupo], descricao: [descricao, Validators.required] }));
  }
  ...
  salvar(): void {
    ...
    const req = this.editando()
      ? this.templateService.atualizarGlobal(this.templateId!, dto)
      : this.templateService.criarGlobal(dto);

    req.subscribe({
      next: () => {
        this.appStore.sucesso(this.editando() ? 'Template atualizado.' : 'Template criado.');
        this.router.navigate(['/templates-globais']);
      },
